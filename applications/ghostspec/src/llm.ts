 ...
 * signing up for anything. Which of the two you have is checked in cli.ts before
 * a browser session is opened — this is the only model call path, and it first
 * runs several billed seconds into exploration.
 */
import { spawn } from "node:child_process"
import { envVar } from "./session.js"

const MODEL = envVar("GHOSTSPEC_MODEL") ?? "claude-sonnet-5"

export async function ask(prompt: string): Promise<string> {
  const key = envVar("ANTHROPIC_API_KEY")
  return key ? viaApi(prompt, key) : viaCli(prompt)
}

async function viaApi(prompt: string, key: string): Promise<string> {
