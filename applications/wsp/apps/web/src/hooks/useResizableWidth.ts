// Adapted from pingdotgg/t3code apps/web/src/hooks/useResizableWidth.ts at 57a66608 (MIT).
import { type PointerEvent as ReactPointerEvent, useCallback, useRef, useState } from "react";

import { finiteNumber, getLocalStorageItem, setLocalStorageItem } from "./useLocalStorage";
...
export function useResizableWidth(options: UseResizableWidthOptions): {
  ...
  const [width, setWidth] = useState<number>(() => {
    ...
    try {
      const stored = getLocalStorageItem(storageKey, finiteNumber);
  ...
  const onPointerUp = useCallback(
    (event: ReactPointerEvent<HTMLElement>) => {
      ...
      try {
        setLocalStorageItem(storageKey, finalWidth, finiteNumber);
      } catch (error) {
