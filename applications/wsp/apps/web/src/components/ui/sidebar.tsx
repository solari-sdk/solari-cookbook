...
function SidebarGroupAction({ className, render, ...props }: useRender.ComponentProps<"button">) {
  const defaultProps = {
    className: cn(
      "absolute top-1/2 right-1.5 flex aspect-square w-5 -translate-y-1/2 items-center justify-center rounded-lg p-0 text-sidebar-foreground outline-hidden ring-ring transition-transform hover:bg-sidebar-row-hover hover:text-sidebar-foreground focus-visible:ring-2 [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
...
function SidebarMenuAction({
  ...
  const defaultProps = {
    className: cn(
      // Centred on the row and on the column a row's own chevron stands in: 6 px in plus half of 20 is the 8 px padding plus half a 16 px glyph.
      "absolute top-1/2 right-1.5 flex aspect-square w-5 -translate-y-1/2 cursor-pointer items-center justify-center rounded-lg p-0 text-sidebar-foreground outline-hidden ring-ring transition-transform hover:bg-sidebar-row-hover hover:text-sidebar-foreground focus-visible:ring-2 peer-hover/menu-button:text-sidebar-foreground [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
      // Increases the hit area of the button on mobile.
      "after:-inset-2 after:absolute md:after:hidden",
      "group-data-[collapsible=icon]:hidden",
      showOnHover &&
