...
SOURCE = SourceDescriptor(
    ...
    capabilities=["events", "geospatial", "public-api", "geojson", "multi-hazard", "deterministic-normalization"],
    depends_on=[],
)
DEFAULT_URL = "https://www.gdacs.org/gdacsapi/api/Events/geteventlist/EVENTS4APP"
MAX_RESPONSE_BYTES = 5 * 1024 * 1024

_HAZARDS = {"EQ": "earthquake", "TC": "tropical-cyclone", "FL": "flood", "VO": "volcano", "DR": "drought", "WF": "wildfire", "TS": "tsunami"}
...
def _point(feature: dict[str, Any]) -> GeoPoint | None:
    ...
    if not (-180 <= longitude <= 180 and -90 <= latitude <= 90):
        return None
    return GeoPoint(latitude=latitude, longitude=longitude, precision="GDACS event centroid")
...
def normalize(payload: dict[str, Any], acquisition_id: str) -> list[EventRecord]:
    ...
    for index, feature in enumerate(payload["features"]):
        ...
        report_url = (url_data.get("report") or url_data.get("details")) if isinstance(url_data, dict) else None
        events.append(EventRecord(
            ...
            evidence=[EvidenceReference(acquisition_id=acquisition_id, field="*", kind=EvidenceKind.OBSERVED, source_path=f"features[{index}]", note="Observed in the public GDACS GeoJSON event API.")],
        ))
    return events
...
def collect(timeout_seconds: int = 20) -> tuple[AcquisitionEnvelope, list[EventRecord]]:
    ...
    with urlopen(request, timeout=timeout_seconds) as response:  # nosec B310 - fixed GDACS HTTPS public API
        ...
        completed = utc_now()
        acquisition = AcquisitionEnvelope(id=acquisition_id, source_id=SOURCE.id, method=SOURCE.method, requested_url=DEFAULT_URL, final_url=response.geturl(), started_at=started, completed_at=completed, status="success", http_status=getattr(response, "status", 200), content_type=response.headers.get("Content-Type"), content_sha256=sha256(raw).hexdigest(), metadata={"response_bytes": len(raw)})
    parser_started = time.perf_counter()
    payload = json.loads(raw)
    if not isinstance(payload, dict):
        raise ValueError("GDACS response must be an object")
    events = normalize(payload, acquisition_id)
    received = len(payload.get("features", [])) if isinstance(payload.get("features"), list) else 0
    acquisition.metadata.update({"parser_duration_ms": (time.perf_counter() - parser_started) * 1000.0, "records_received": received, "records_accepted": len(events), "records_rejected": max(0, received - len(events))})
