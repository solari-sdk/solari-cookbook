...
app = FastAPI(title="Solari OSINT Operations Center", version="0.3.0", description="Public-source OSINT operations dashboard and Solari execution showcase.")
STATIC_DIR = Path(__file__).parent / "static"; app.mount("/static", StaticFiles(directory=STATIC_DIR), name="static")
ADAPTERS = {x.SOURCE.id: x for x in (usgs_earthquakes, nws_alerts, swpc_alerts)}
...
def health() -> dict[str, object]: return {"status":"ok","service":"solari-osint-operations-center","version":"0.3.0","sources_registered":len(SOURCES)}
...
def health_by_source() -> list[dict[str, object]]: return source_health()

@app.get("/api/v1/acquisitions")
def acquisitions(limit: int=100, source_id: str|None=None) -> list[dict[str, object]]:
