...
export function graphLayout(entities,relationships,{width=720,height=520,maxNodes=100,maxEdges=250}={}){
  const allNodes=[...(entities||[])];
  const nodes=allNodes.slice(0,maxNodes);
  const ids=new Set(nodes.map((node)=>node.id));
  const validEdges=(relationships||[]).filter((edge)=>{const[source,target]=ends(edge);return ids.has(source)&&ids.has(target);});
  const edges=validEdges.slice(0,maxEdges);
  ...
  return{nodes:positioned,edges,width,height,truncated:allNodes.length>nodes.length||validEdges.length>edges.length};
}
